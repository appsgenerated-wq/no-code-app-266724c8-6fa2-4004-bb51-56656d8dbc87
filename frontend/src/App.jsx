...
function App() {
  ...
  const createMonkey = async (monkeyData) => {
    try {
      ...
      // Refetch to get the populated relationships
      const createdMonkey = await manifest.from('Monkey').read(newMonkey.id, { include: ['owner', 'troop'] });
      setMonkeys([createdMonkey, ...monkeys]);
    } catch (error) {
